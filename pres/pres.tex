...
\begin{document}
	...
	\begin{frame}{Функциональная верификация}
		...
		\begin{block}{Вызовы}
			\begin{itemize}
				...
				\item Обнаружение ошибок как можно быстрее
			\end{itemize}
		\end{block}
	...
	\begin{frame}{Постановка задачи}
		...
		Разрабатывается L2 кэш --- один из ключевых компонентов.
		\begin{block}{Цель работы:}
			\begin{itemize}
				\item Верификация L2 кэша GPGPU
			\end{itemize}
		\end{block}
		
		\begin{block}{Задачи:}
			\begin{itemize}
				\item Применить методы формальной верификации и симуляции к L2 кэшу
				\item Провести сравнительный анализ рассматриваемых методов
			\end{itemize}
		\end{block}
		
	\end{frame}
	
	\begin{frame}{Методы, основанные на симуляции: основы}
	...
	\begin{frame}{Методы, основанные на симуляции: ключевые особенности}
	...
	\begin{frame}{Формальная верификация: основы}
		...
		\begin{block}{Алгоритм:}
			\begin{enumerate}
				...
				\item[3.] Считаем произведение $J$ и проверяющего автомата.
	...
	\begin{frame}{Формальная верификация: проблема комбинаторного взрыва}
	...
	\begin{frame}{Формальная верификация: ключевые особенности}
		...
		\begin{block}{Применима только к синхронным схемам.}
		...
		\begin{columns}
			\begin{column}{0.5\textwidth}
				\begin{block}{Обозначения:}
					\begin{itemize}
						\item DD (Data Decode): приём пакетов с L1
						\item T (Tag): наличие кэш-линии
						\item S (Schedule): окончательное "расписание"
						\item D (Data): хранение данных кэша
						\item Q (Queue): буферизация запросов в память
						\item M (Miss): обработка miss
						\item C (Cross): передача пакетов в L1
					\end{itemize}
				\end{block}
			\end{column}
			...
			\begin{column}{0.6\textwidth}
				\begin{block}{}
					\begin{figure}[htb]
						\includegraphics[width=1\textwidth]{../src/img/l2.png}
			...
			\begin{column}{0.5\textwidth}
				\begin{block}{Формальная верификация:}
					\begin{itemize}
						...
						\item Решение: дополнительная логика + разделение блока на части
	...
	\begin{frame}{Сравнение методов}
		...
		\small
			\begin{tabular}{l|r|r|r|r|r}
				...
				$C_{\text{max}}$ & 98.67\% & 94.94\% & 97.85\% & 99.63\% & 95.06\% \\ \hline 
				\rowcolor{lime} $t_{\text{сим}}$, с & 10                 & 12      & 10       & 201     & 19      \\ \hline
				\rowcolor{yellow} $t_{\text{форм}}$, мин                         & 4694 & 1031    & 46       & 3752    & 720     \\ 
				
			\end{tabular}
			\caption{Сравнение методов формальной верификации и UVM-тестирования.}
			\label{tab:main-table}
		\end{table}
		
		\begin{block}{Примеры найденных ошибок:}
			\begin{itemize}
				\item D-stage: не проверялась валидность инструкций, записывающих данные в память. 
				\item T-stage: некорректно выставлялся флаг для пакетов, передающихся на S-stage. 
				\item С-stage: неправильная арбитрация пакетов, приходящих с M-, D-stage и TLB.
			\end{itemize}
		\end{block}
	\end{frame}
	...
	\begin{frame}{Выводы}
		...
		\begin{block}{Преимущества симуляции:}
			\begin{itemize}
				...
				\item быстрая верификация больших систем
			\end{itemize}
		\end{block}
	...
	\begin{frame}{Заключение}
		...
		\begin{block}{Реализованы методы формальной верификации и симуляции для L2 кэша GPGPU:}
			\begin{itemize}
				...
				\item формальная верификация --- для средних и малых блоков, также для критических компонент
			\end{itemize}
		\end{block}
